...
impl Png {
    ...
    /// Creates a `Png` from a `Vec<Chunk>`
    pub fn from_chunks(chunks: Vec<Chunk>) -> Self {
        Self { chunks }
    }

    /// Adds a `Chunk` to a `Png`
    ...
    /// Returns a slice of the chunks contained in the `Png`
    pub fn chunks(&self) -> &[Chunk] {
        &self.chunks[..]
    }

    /// Finds and returns the first `Chunk` with type `chunk_type`
    ...
    /// Returns a `Vec<u8>` of the bytes that represent the `Png`
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        ensure!(value.len() >= 8, "Not enough bytes to make a Png");
        ensure!(
            value[0..8] == Png::STANDARD_HEADER,
            "Header is not PNG header"
        );
